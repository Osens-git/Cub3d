...
/*   Updated: 2025/06/23 11:15:00 by vluo             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_player(t_data *data, t_pos *pos_init, char dir)
{
	...
	player -> near_door = init_pos(0, 0);
	if (!player -> pos || !player -> d || !player -> near_door)
		return (free(player->pos), free(player->d),
			free(player->near_door), free(player));
	data -> p = player;
	return (data->p->k_w = 0, data->p->k_a = 0, data->p->k_s = 0,
		data->p->k_d = 0, data->p->k_l = 0, data->p->k_r = 0, (void)dir);
}
